...
if [ -e ./ecoli_60.ms22.histogram -a -e ./ecoli_60.ms22.dump -a -e ./ecoli_60.ms22.ignore.gz ] ; then
  exit 0
fi
...
if [ -e ecoli_60.ms22.histogram ]; then
  exists1=true
else
  exists1=false
fi

if [ -e ecoli_60.ms22.dump ]; then
  exists2=true
else
  exists2=false
fi

if [ -e ecoli_60.ms22.ignore.gz ]; then
  exists3=true
else
  exists3=false
fi
if [ $exists1 = true -a $exists2 = true -a $exists3 = true ] ; then
  echo "Output files 'ecoli_60.ms22.histogram', 'ecoli_60.ms22.dump' and 'ecoli_60.ms22.ignore.gz' exist in 'trimming/0-mercounts'."
  exit 0
fi
...
if [ ! -e ./ecoli_60.ms22/merylIndex ] ; then
  /Users/pfb2024/canu-2.2/bin/meryl threads=4 memory=2 \
    greater-than 1 \
      output ecoli_60.ms22.WORKING \
      union-sum  \
        ./ecoli_60.01.meryl \
  && \
  mv -f ./ecoli_60.ms22.WORKING ./ecoli_60.ms22
...
if [ ! -e ./ecoli_60.ms22.histogram ] ; then
  /Users/pfb2024/canu-2.2/bin/meryl threads=1 memory=1 \
    statistics ./ecoli_60.ms22 \
  > ./ecoli_60.ms22.histogram.WORKING \
  && \
  mv ./ecoli_60.ms22.histogram.WORKING ./ecoli_60.ms22.histogram
fi
...
if [ ! -e ./ecoli_60.ms22.dump ] ; then
  /Users/pfb2024/canu-2.2/bin/meryl threads=4 memory=2 \
    print ./ecoli_60.ms22.##.dump \
      at-least distinct=0.9999 \
        ./ecoli_60.ms22

  cat ./ecoli_60.ms22.??.dump > ./ecoli_60.ms22.dump
  rm -f ./ecoli_60.ms22.??.dump
fi
...
if [ ! -e ./ecoli_60.ms22.ignore.gz ] ; then
  numKmers=`wc -l < ./ecoli_60.ms22.dump`
  totKmers=`/Users/pfb2024/canu-2.2/bin/meryl statistics ./ecoli_60.ms22 | grep present | awk '{ print $2 }'`


  ./meryl-make-ignore.pl $numKmers $totKmers < ./ecoli_60.ms22.dump | gzip -1c > ./ecoli_60.ms22.ignore.gz
fi
