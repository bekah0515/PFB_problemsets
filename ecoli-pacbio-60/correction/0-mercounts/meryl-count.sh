...
if [ -e ./ecoli_60.$jobid.meryl/merylIndex ] ; then
...
if [ -e ecoli_60.$jobid.meryl.tar.gz ]; then
  exist1=true
else
  exist1=false
fi
...
/Users/pfb2024/canu-2.2/bin/meryl k=16 threads=4 memory=2 \
  count \
    segment=$jobid/01 ../../ecoli_60.seqStore \
    output ./ecoli_60.$jobid.meryl.WORKING \
&& \
mv -f ./ecoli_60.$jobid.meryl.WORKING ./ecoli_60.$jobid.meryl
